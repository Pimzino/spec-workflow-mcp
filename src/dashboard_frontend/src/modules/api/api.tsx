...
type ApiActionsContextType = {
  ...
  updateTaskStatus: (specName: string, taskId: string, status: 'pending' | 'in-progress' | 'completed' | 'blocked') => Promise<{ ok: boolean; status: number; data?: any }>;
...
export function ApiProvider({ initial, projectId, children }: ApiProviderProps) {
  ...
  const actionsValue = useMemo<ApiActionsContextType>(() => {
    ...
    return {
      ...
      updateTaskStatus: (specName: string, taskId: string, status: 'pending' | 'in-progress' | 'completed' | 'blocked') =>
