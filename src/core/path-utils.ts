...
export class PathUtils {
  ...
  private static safeJoin(basePath: string, ...paths: string[]): string {
    ...
    for (const pathSegment of paths) {
      if (
        pathSegment &&
        (pathSegment.includes("..") || pathSegment.startsWith("/"))
      ) {
  ...
  static getOriginRequirementsPath(projectPath: string): string {
    return this.safeJoin(projectPath, ".spec-workflow", "origin-requirements");
  }
...
export async function validateProjectPath(
  ...
  try {
    ...
    const allSystemPaths =
      process.platform === "win32" ? windowsSystemPaths : systemPaths;

    for (const sysPath of allSystemPaths) {
      if (absolutePath.toLowerCase().startsWith(sysPath.toLowerCase())) {
        throw new Error(
          `Access to system directory not allowed: ${absolutePath}`
        );
      }
    }
    ...
    if (error instanceof Error) {
      if ((error as any).code === "ENOENT") {
        ...
        throw new Error(
          `Permission denied accessing project path: ${projectPath}`
        );
      }
      throw error;
    }
...
export async function ensureWorkflowDirectory(
  projectPath: string
): Promise<string> {
