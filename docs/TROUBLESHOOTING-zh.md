...
本指南帮助你解决 Spec Workflow MCP 的常见问题。
...
# 验证 .spec-workflow 目录存在
ls -la .spec-workflow  # 或 Windows 上的 'dir .spec-workflow'
```
...
2. **Claude Code CLI**: 验证设置:
   ...
   claude mcp remove spec-workflow  # 如存在则删除
...
3. 验证配置已正确保存
4. 尝试明确提及工具："使用 spec-workflow 创建规范"
...
**错误**: `WebSocket connection lost` 或实时更新不工作

**解决方案**:
1. 刷新浏览器页面
2. 检查防火墙是否阻止 WebSocket
3. 验证仪表板和 MCP 服务器从同一项目运行
4. 检查浏览器控制台中的特定错误
5. 尝试不同网络（如在公司网络上）
...
**错误**: 创建规范时出现 `EACCES` 或 `Permission denied`
