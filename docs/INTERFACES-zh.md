...
1. **Web 仪表板** - 面向 CLI 用户的基于浏览器的界面
2. **VSCode 扩展** - 面向 VSCode 用户的集成 IDE 体验

两个界面提供相同的核心功能，并针对各自平台进行了优化。

## Web 仪表板

### 概述

Web 仪表板是一个实时 Web 应用程序，提供对规范、任务和审批工作流的可视化访问。
...
| `archive.showInExplorer` | 显示已归档规范 | true |
...
| 命令 | 描述 |
|---------|-------------|
| `Spec Workflow: Create Spec` | 开始新规范 |
| `Spec Workflow: List Specs` | 显示所有规范 |
| `Spec Workflow: View Dashboard` | 打开 Web 仪表板 |
| `Spec Workflow: Archive Spec` | 移至归档 |
| `Spec Workflow: Restore Spec` | 从归档恢复 |
| `Spec Workflow: Refresh` | 重新加载规范数据 |
...
| 问题 | 解决方案 |
|-------|----------|
| 未显示规范 | 检查项目是否有 .spec-workflow 目录 |
| 命令不工作 | 重新加载 VSCode 窗口 |
| 无通知 | 检查扩展设置 |
| 归档不可见 | 在设置中启用 |
...
- [用户指南](USER-GUIDE.md) - 使用界面
- [工作流程](WORKFLOW.md) - 开发工作流
