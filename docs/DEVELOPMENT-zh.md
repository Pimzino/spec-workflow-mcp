...
这会将 TypeScript 文件编译为 `dist/` 目录中的 JavaScript。
...
```typescript
console.error('[DEBUG]', '工具调用:', toolName, params);
```
...
| 模块未找到 | 检查导入并运行 `npm install` |
| 端口已被占用 | 更改端口或终止现有进程 |
| WebSocket 连接失败 | 检查服务器是否运行且端口正确 |

### 开发提示

1. **使用 TypeScript 严格模式** 获得更好的类型安全
2. **启用源映射** 便于调试
3. **使用 nodemon** 在开发期间自动重启
4. **在隔离目录中测试文件操作**
5. **使用 Chrome DevTools 监控性能**
...
- [配置指南](CONFIGURATION.md) - 服务器配置
- [用户指南](USER-GUIDE.md) - 使用服务器
- [工具参考](TOOLS-REFERENCE.md) - 工具文档
